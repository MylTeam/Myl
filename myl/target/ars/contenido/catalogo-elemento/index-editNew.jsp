...
<body>
	<s:url id="urlCancelar"
		value="/catalogo-elemento.action?idExpoSel=%{idExpoSel}"
		includeContext="true" />
	<s:actionerror id="saeElemento" theme="jquery" />
	<s:fielderror id="sfeElemento" theme="jquery" />

	<s:form
		action="%{#request.contextPath}/catalogo-elemento.action?idExpoSel=%{idExpoSel}"
		method="post" theme="simple" acceptcharset="UTF-8" id="frmElemento"
		enctype="multipart/form-data" style="border: 0px">
		<center>
				...
				<s:iterator value="recursosTemporales">
					<tr>
						...
						<td width="60"><input type="button"
							onclick="cambiaEditar('${nombre}','${descripcion}','${idTemporal}');"
							value="editar" /> <input type="button"
							onclick="cambiaEliminar('${nombre}','${descripcion}','${idTemporal}');"
							value="eliminar" /></td>
					</tr>
				</s:iterator>
			</tbody>
					...
					<td colspan="4" style="text-align: center;"><input id="btnNew"
						...
						style="width: 150px;" /> <sj:dialog id="mydialog" title="Recurso"
							...
							<s:form
								...
								<sj:a id="btnCancelar2" button="true" buttonIcon="ui-icon-bullet"
									href="#" 
